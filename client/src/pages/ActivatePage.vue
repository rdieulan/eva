...
onMounted(() => {
  ...
  if (!token.value) {
    setError(ERROR.activationTokenRequired);
  }
});
...
async function handleActivate() {
  ...
  try {
    await activateAccount(token.value, password.value);
    success.value = true;
    setTimeout(() => {
    ...
    if (err && typeof err === 'object' && 'errors' in err) {
      ...
      setError(ERROR.activationFailed);
    }
  } finally {
...
<template>
  <div class="activate-page">
    <div class="activate-card">
      ...
      <form v-else @submit.prevent="handleActivate">
        <p class="info-text">
